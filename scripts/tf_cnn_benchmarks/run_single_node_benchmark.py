...
def main():
  ...
  if not os.path.isfile(script):
    sys.exit("Could not find the python script. Please make sure that mkl_tf_cnn_benchmarks.py is in the same directory as run_single_node_benchmark.py.")
  arg_parser.add_argument("--file_location", help='<path to script file>', default=script)
  # CBR: Per Ashraf's request changing model and cpu back to optional parameters with bdw and alexnet default values
  ...
  arg_parser.add_argument('-o', "--num_omp_threads", help='Specify the number of OpenMP threads', type=int, dest="num_omp_threads", default=None)
  ...
  arg_parser.add_argument('-p', "--ps_hosts", help="List of parameter servers by IP addresses or hostnames", dest="ps_hosts", default=None)
  arg_parser.add_argument('-w', "--worker_hosts", help="List of worker servers by IP addresses or hostnames", dest="worker_hosts", default=None)
  ...
  arg_parser.add_argument("--forward_only", help="Only run inference.", dest="forward_only", default=False)
  ...
  command_prefix = command_prefix + (' --model {model}'
      ...
      ' --num_warmup_batches {num_warmup_batches}').format(
      model=args.model,
      cpu=args.cpu,
