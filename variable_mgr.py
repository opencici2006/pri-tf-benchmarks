...
class VariableMgrIndependent(VariableMgr):
  ...
  """
    ...
    def get_gradients_to_apply(self, device_num, gradient_state):
        ...
        return [
            grad_and_vars[device_num] for grad_and_vars in zip(*device_grads)
        ]
  ...
  """
    ...
    def get_devices(self):
        ...
        if self.benchmark_cnn.local_parameter_device_flag == 'gpu':
            return [
                ParamServerDeviceSetter(d, raw_devices) for d in raw_devices
            ]
        else:
            return [
  ...
  """
    ...
    def get_devices(self):
        ...
        return [
            tf.train.replica_device_setter(
                worker_device=d,
                cluster=self.benchmark_cnn.cluster,
    ...
    def get_post_init_ops(self):
        ...
        for v in tf.global_variables():
            if v.name.startswith(PS_SHADOW_VAR_PREFIX + '/v0/'):
                prefix = strip_port(v.name[len(PS_SHADOW_VAR_PREFIX + '/v0'):])
