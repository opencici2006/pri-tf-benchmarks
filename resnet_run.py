...
tf.flags.DEFINE_string('variable_update', 'parameter_server', (
    'The method for managing variables: '
        ...
        with tf.variable_scope(name):
            ...
            if mode != 'SAME_RESNET':
                ...
                if d_height == 1 and d_width == 1:
                    ...
                    kernel_size_effective = k_height + (k_width - 1) * (
                        rate - 1)
                    pad_total = kernel_size_effective - 1
                    pad_beg = pad_total // 2
                    pad_end = pad_total - pad_beg
                    padding = [[0, 0], [pad_beg, pad_end], [pad_beg, pad_end],
                               [0, 0]]
                    if self.data_format == 'NCHW':
                        padding = [
    ...
    def affine(self,
        ...
        with tf.variable_scope(name):
            ...
            kernel = tf.get_variable(
                ...
                self.data_type,
                tf.random_normal_initializer(stddev=np.sqrt(
                    init_factor / (num_channels_in))))
            biases = tf.get_variable('biases', [num_out_channels],
                                     self.data_type,
                                     tf.constant_initializer(0.0))
...
class BenchmarkCNN(object):
    ...
    def __init__(self):
        ...
        self.model_conf = resnet_model.Resnetv1Model(self.model, (3, 4, 6, 3))
        ...
        with tf.Session(target=target, config=create_config_proto()) as sess:
            ...
            for step in xrange(self.num_batches):
                ...
                if (step + 1) % FLAGS.display_every == 0:
                    ...
                    log_fn('%i\t%.1f examples/sec' % (step + 1,
                                                      examples_per_sec))
                    start_time = time.time()
            ...
            summary = tf.Summary()
            summary.value.add(
                tag='eval/Accuracy@1', simple_value=precision_at_1)
    ...
    def _benchmark_cnn(self):
        ...
        if self.job_name and FLAGS.cross_replica_sync:
            ...
            fetches.append(
                self.add_sync_queues_and_barrier('sync_queues_step_end_',
                                                 [main_fetch_group]))
        ...
        summary_writer = None
        if (is_chief and FLAGS.summary_verbosity and FLAGS.train_dir and
        ...
        for device_num in range(len(self.devices)):
            with self.variable_mgr.create_outer_variable_scope(
                ...
                results = self.add_forward_pass_and_gradients(
                    ...
                    nclass, phase_train, device_num, input_data_type,
                    data_type, input_nchan, use_synthetic_gpu_images,
    ...
    def add_forward_pass_and_gradients(
        ...
        with tf.device(self.devices[device_num]):
            ...
            network = ConvNetBuilder(images, input_nchan, phase_train,
    ...
    def add_sync_queues_and_barrier(self, name_prefix, enqueue_after_list):
        ...
        with tf.device(self.sync_queue_devices[self.sync_queue_counter % len(
            self.sync_queue_devices)]):
            sync_queues = [
                tf.FIFOQueue(
                    num_workers, [tf.bool],
